...
class Config: NSObject, NSCoding {
    ...
    fileprivate func updateConfiguration() {
        ...
        var request = URLRequest(url: appDelegate.tmdbURLFromParameters(methodParameters as [String : AnyObject], withPathExtension: "/configuration"))
        ...
        let task = appDelegate.sharedSession.dataTask(with: request, completionHandler: { (data, response, error) in
            ...
            do {
                parsedResult = try JSONSerialization.jsonObject(with: data, options: JSONSerialization.ReadingOptions.allowFragments) as! [String :AnyObject] as AnyObject!
            } catch {
